...
export default function DetailModal({ item, onClose, onPlayClick, isDemo }: DetailModalProps) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-40 bg-black/90 backdrop-blur-md flex justify-center items-end md:items-center p-0 md:p-6 overflow-y-auto no-scrollbar"
    ...
    >
      ...
      <AnimatePresence>
        {showShareToast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="fixed top-4 left-1/2 -translate-x-1/2 bg-[#E5A00D] text-black font-bold uppercase tracking-widest text-[10px] px-6 py-3 rounded-none shadow-2xl z-50 flex items-center gap-1.5 border border-white/10"
          >
      ...
      <motion.div
        ...
        className="relative w-full max-w-4xl bg-[#080808] border-t md:border border-white/10 rounded-none shadow-2xl overflow-hidden max-h-[85vh] md:max-h-[90vh] flex flex-col focus:outline-none"
      ...
      >
        ...
        <div className="relative h-60 sm:h-80 w-full flex-shrink-0 select-none">
          {backdropSrc ? (
            ...
            <div className="w-full h-full bg-[#080808] border-b border-white/10" />
          )}
          ...
          <div className="absolute bottom-4 left-4 sm:left-8 right-4 sm:right-8 flex flex-col md:flex-row gap-4 md:items-end justify-between">
            <div className="space-y-1 sm:space-y-2 font-sans">
              ...
              {item.genres && item.genres.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {item.genres.slice(0, 3).map((g) => (
                    <span
                      key={g.id}
                      className="text-[9px] sm:text-[10px] bg-[#E5A00D]/10 text-[#E5A00D] px-2.5 py-0.5 border border-[#E5A00D]/10 rounded-none font-bold font-mono tracking-widest uppercase"
                    >
                      {g.name}
                    </span>
                  ))}
              ...
              <h2 className="text-xl sm:text-3xl font-display font-black text-white leading-tight uppercase tracking-tight">
                {title}
              </h2>
              ...
              <div className="flex items-center gap-3 text-xs text-gray-300 font-mono">
                ...
                <div className="flex items-center gap-1.5 text-[#E5A00D] font-bold bg-[#080808]/70 px-1.5 py-0.5 rounded-none border border-white/10">
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 bg-white hover:bg-[#E5A00D] text-black rounded-none text-xs sm:text-sm font-black uppercase tracking-[0.15em] hover:text-black transition-all cursor-pointer shadow-none"
                id="modal-play-btn"
              >
        ...
        <div className="flex-1 p-5 sm:p-8 space-y-6 overflow-y-auto no-scrollbar font-sans border-t border-white/5">
          {/* Overview Section */}
          <div className="space-y-2">
            <h3 className="font-display font-black text-xs text-gray-400 tracking-[0.2em] uppercase">Overview Description</h3>
            <p className="text-sm text-gray-300 leading-relaxed max-w-3xl" id="modal-overview-text">
          ...
          {item.credits?.cast && item.credits.cast.length > 0 && (
            <div className="space-y-3">
              <h3 className="font-display font-black text-xs text-gray-400 tracking-[0.2em] uppercase">Cast Members</h3>
                ...
                {item.credits.cast.slice(0, 8).map((actor: CastMember) => {
                  ...
                  return (
                    <div key={actor.id} className="flex-shrink-0 w-16 text-center space-y-1">
                      <div className="w-12 h-12 rounded-none overflow-hidden mx-auto bg-white/5 border border-white/10">
                        {actorImage ? (
                          <img
                            src={actorImage}
                            alt={actor.name}
                      ...
                      <p className="text-[10px] text-gray-200 font-semibold truncate leading-none uppercase tracking-wide">{actor.name}</p>
                      <p className="text-[8px] text-gray-500 font-mono truncate leading-none">{actor.character}</p>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="space-y-4 pt-4 border-t border-white/10">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-1.5 text-white">
                  ...
                  <h3 className="font-display font-black text-xs uppercase tracking-widest text-[#F5F5F5]">TV Episode Navigation</h3>
                </div>
                ...
                {loadingEpisodes ? (
                  ...
                  <div className="grid grid-cols-1 gap-2" id="modal-episode-list">
                    {seasonEpisodes.map((ep) => (
                      <div
                        key={ep.id}
                        className="bg-[#080808] border border-white/10 hover:border-[#E5A00D]/30 p-3 sm:p-4 rounded-none flex flex-col sm:flex-row gap-4 items-start justify-between transition-all"
                      ...
                      >
                        <div className="flex-1 space-y-1.5 text-left">
                          <div className="flex items-center gap-2">
                            ...
                            <h4 className="text-xs sm:text-xs font-bold uppercase tracking-wide text-gray-100">{ep.name || `Episode ${ep.episode_number}`}</h4>
                          </div>
                          <p className="text-[11px] sm:text-xs text-gray-400/80 leading-relaxed line-clamp-2">
                        ...
                        <button
                          ...
                          className="w-full sm:w-auto px-4 py-2 sm:mt-1 bg-white hover:bg-[#E5A00D] text-black font-bold text-xs rounded-none border border-white/10 hover:border-transparent flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                          id={`modal-play-episode-${ep.episode_number}`}
                        >
                          <Play className="w-3 h-3 fill-current text-black leading-none" />
                          <span>Stream</span>
                        </button>
                      </div>
                    ))}
                  ...
                  <div className="py-6 text-center text-xs text-gray-500 font-mono uppercase tracking-widest">Wait list is being fetched or season is unreleased.</div>
                )}
              </div>
            </div>
          )}
          ...
          {item.recommendations?.results && item.recommendations.results.length > 0 && (
            <div className="space-y-3 pt-4 border-t border-white/10">
              <div className="flex items-center gap-1.5 text-white">
                ...
                <h3 className="font-display font-black text-xs text-gray-400 tracking-[0.2em] uppercase">More Like This</h3>
              </div>
                ...
                {item.recommendations.results.slice(0, 8).map((rec, index) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex-shrink-0 w-24 bg-white/5 border border-white/10 hover:border-[#E5A00D] p-1.5 rounded-none text-left space-y-1.5 transition-all text-xs active:scale-95 cursor-pointer"
                      id={`modal-rec-${rec.id}`}
                    >
                      ...
                      <p className="font-semibold text-gray-200 truncate uppercase tracking-widest text-[9px] leading-none">{recTitle}</p>
                    </button>
                  );
                })}
              </div>
